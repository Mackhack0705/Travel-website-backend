...
import hotels from "../models/hotelSchema.js"; // Import the 'hotels' model
...
router.post("/add", async (req, resp) => {
    try {
        ...
        const newItem = new hotels(req.body);
...
router.get("/search", async (req, resp) => {
    try {
        ...
        // Use the hotels model to find items based on the query
        const items = await hotels.find(query);
...
router.put("/update/:_id", async (req, resp) => {
    try {
        ...
        const item = await hotels.findById(itemId);
...
router.delete("/delete/:_id", async (req, resp) => {
    try {
        ...
        const deletedItem = await hotels.findOneAndRemove({
            _id: itemId,
        });
